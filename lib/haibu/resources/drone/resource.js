...
//
Drone.before('create', function setupEnvironment(obj, next) {
    var self = obj;
    console.error('fetching env')
    function fetch(environmentId) {
        console.error(environmentId, 123)
        haibu.resources.Environment.get(
            haibu.resources.Environment.url({name: environmentId}), function (err, environment) {
            if (err) {
                next(err);
                return;
            }
    ...
    console.error(self.environment)
    if (typeof self.environment === 'string') {
        fetch(self.environment);
    }
    else if (!(self.environment instanceof haibu.resources.Environment)) {
        try {
            fetch(haibu.resources.Environment.url(self.environment));
        }
        catch (e) {
            next(e);
        }
...
Drone.before('create', function setupRepository(obj, next) {
    console.log(arguments)
    var self = obj;
    function fetch(repositoryId) {
        haibu.resources.Repository.get(repositoryId, function (err, repository) {
            if (err) {
                next(err);
                return;
            }
    ...
    if (typeof self.repository === 'string') {
        fetch(self.repository);
    }
    else if (!(self.repository instanceof haibu.resources.Repository)) {
        try {
            fetch(haibu.resources.Repository.url(self.repository));
        }
        catch (e) {
            next(e);
        }
...
Drone.url = function (params) {
    return '/environment/' + params.environment.name
        + '/user/' + params.repository.application.user
        + '/application/' + params.repository.application.name
        + '/repository/' + params.repository.name
        + '/drone/' + (params.monitor && params.monitor.pid);
}

Object.defineProperty(Drone.prototype, '_id', {
    enumerable: true,
    configurable: false,
    get: function getId() {
        return Drone.url(this);
    }
});
...
Drone.prototype.bootstrap = function bootstrap(callback) {
    var self = this;
    this.getForeverOptions(function configured(err, foreverOptions) {
        console.error('options', arguments)
        if (err) {
            callback(err);
            return;
        }
